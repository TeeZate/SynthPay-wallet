      ...
      <div style={{ maxWidth:960, margin:'0 auto', padding:'48px 24px' }}>
        ...
        <div style={{ position:'relative', marginBottom:16 }}>
          ...
          <input
            ...
            style={{ width:'100%', padding:'12px 14px 12px 42px', borderRadius:12, border:'1px solid #1a1a2e', background:'#0f0f1a', color:'#e2e8f0', fontSize:14, outline:'none', boxSizing:'border-box', fontFamily:'system-ui,sans-serif' }}
          />
          {search && (
            <button onClick={() => setSearch('')} style={{ position:'absolute', right:14, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', color:'#64748b', cursor:'pointer', fontSize:18, lineHeight:1 }}>×</button>
          )}
        </div>

        {/* Category filter pills */}
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:36, overflowX:'auto', paddingBottom:4 }}>
          {ALL_CATEGORIES.map(cat => {
            const active = activeCategory === cat
            const cfg    = cat !== 'All' ? getConfig(cat) : null
            return (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                style={{ padding:'6px 16px', borderRadius:20, border:`1px solid ${active ? (cfg?.accent || '#00e5ff') : '#1a1a2e'}`, background: active ? `${cfg?.accent || '#00e5ff'}18` : 'transparent', color: active ? (cfg?.accent || '#00e5ff') : '#64748b', fontSize:12, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap', transition:'all 0.15s' }}>
                {cat !== 'All' && cfg ? `${cfg.icon} ` : ''}{cat}
              </button>
            )
          })}
        </div>
        ...
        {!loading && merchants.length > 0 && filteredMerchants.length === 0 && (
          <div style={{ textAlign:'center', padding:60, background:'#0f0f1a', borderRadius:16, border:'1px dashed #1a1a2e' }}>
            <p style={{ fontSize:28, marginBottom:12 }}>🔍</p>
            <p style={{ color:'#334155', fontSize:14 }}>No results for "{search || activeCategory}"</p>
            <button onClick={() => { setSearch(''); setActiveCategory('All') }} style={{ marginTop:12, padding:'8px 20px', borderRadius:8, border:'1px solid #1a1a2e', background:'none', color:'#64748b', cursor:'pointer', fontSize:13 }}>Clear filters</button>
          </div>
        )}
...
function ServiceCard({ merchant, service, balance, unlocked, onPay, featured }: { merchant: Merchant; service: Service; balance: number; unlocked: boolean; onPay: () => void; featured?: boolean }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
         style={{ background: hovered ? config.bg : '#0a0a14', border:`1px solid ${hovered ? config.accent+'40' : featured ? config.accent+'30' : '#0d0d1e'}`, borderRadius:16, padding:24, transition:'all 0.25s ease', transform: hovered ? 'translateY(-4px)' : 'none', boxShadow: hovered ? `0 12px 40px ${config.accent}18` : featured ? `0 0 0 1px ${config.accent}20` : 'none', position:'relative', overflow:'hidden' }}>
      {(hovered || featured) && <div style={{ position:'absolute', top:0, left:0, right:0, height:2, background:`linear-gradient(90deg, transparent, ${config.accent}, transparent)` }} />}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          {featured && <span style={{ fontSize:9, letterSpacing:2, color:'#f59e0b', fontFamily:'monospace', background:'rgba(245,158,11,0.1)', border:'1px solid rgba(245,158,11,0.3)', padding:'2px 6px', borderRadius:4 }}>FEATURED</span>}
        </div>
